namespace RFI.AdventOfCode2024.Day1;
...
/// 
/// You haven't even left yet and the group of Elvish Senior Historians has already hit a problem: their list of locations to 
/// check is currently empty. Eventually, someone decides that the best place to check first would be the Chief Historian's office.
/// 
...
/// an assortment of notes and lists of historically significant locations! This seems to be the planning the Chief Historian was doing 
...
/// Or are they?
/// 
/// The Historians can't agree on which group made the mistakes or how to read most of the Chief's handwriting, but in the commotion you notice 
/// an interesting detail: a lot of location IDs appear in both lists! Maybe the other numbers aren't location IDs at all but rather misinterpreted handwriting.
/// 
...
/// </summary>
public class ListReconciler
{
    public int Reconcile(IEnumerable<int> list1, IEnumerable<int> list2)
    {
        if (list1.Count() != list2.Count())
        {
            throw new ArgumentException("The lists are of different length.");
        }

        var ordered1 = list1.OrderBy(x => x).ToArray();
        var ordered2 = list2.OrderBy(x => x).ToArray();

        var sum = 0;
        for (int i = 0; i < ordered1.Length; i++)
        {
            sum += Math.Abs(ordered1[i] - ordered2[i]);
        }

        return sum;
    }

    public int CalculateSimilarityScore(IEnumerable<int> list1, IEnumerable<int> list2)
        => list1
            .GroupBy(x => x)
            .Sum(g => g.Key * g.Count() * list2.Count(x => x == g.Key));
}
